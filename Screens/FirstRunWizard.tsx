...
export default function FirstRunWizard({ navigation }: FirstRunProps) {
  ...
  const FirstRunSchema = Yup.object().shape({
    ...
    phone: Yup.string().required('Phone number is required').min(14, 'Invalid phone number'),
    ...
    availableDays: Yup.array()
      ...
      .test('atLeastOneDay', 'You must check at least one day', function (value) {
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        storeData(values.email, values).then(() => state.setUser(values));
      }}
...
function checkErrors(errors: FormikErrors<FirstRunValues>, currentStep) {
  ...
  if (currentStep === 5 && !errors.operatingHoursStart && !errors.operatingHoursEnd) {
    return false;
  }
  if (currentStep === 6) {
    return false;
